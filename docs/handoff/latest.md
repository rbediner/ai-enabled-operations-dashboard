...
**Last updated:** 2026-04-16 — Session: Repo cleanup & restructure (file organization, .gitignore, npm run screenshot, canopy-exec-dash-prt documented)
...
2. Do not restart or redesign the visual system — V2.7 is stable
3. If continuing visual polish: address "What Still Needs Work" items in order
4. If starting V3: begin with T2 mode label wiring (simplest, highest meaning value)
5. After every session: run `npm run screenshot` — all 3 states must be captured and saved
...
```
canopy-exec-dash-prt/              ← SEPARATE git repo — the public GitHub Pages build
                                      Purpose: static deploy target for stakeholder review
                                      Repo: github.com/rbediner/canopy-exec-dash-prt
                                      Has its own node_modules/ and git history
                                      Do NOT edit src files here — work in the main project
design/
...
scripts/
  capture-dashboard-screenshots.mjs   ← Puppeteer script: captures all 3 dashboard states
...
| GitHub Pages | [rbediner.github.io/canopy-exec-dash-prt](https://rbediner.github.io/canopy-exec-dash-prt) | Live |

GitHub Pages deploys via `.github/workflows/deploy-pages.yml` (Node 22).

**The public GitHub Pages repo** lives at `canopy-exec-dash-prt/` inside this project folder. It is a separate git repo with its own `node_modules/` — do not confuse it with the main project.
...
| **2026-04-16 (pass 2)** | `dist/` deleted; `.gitignore` added; `npm run screenshot` script added to `package.json` |
